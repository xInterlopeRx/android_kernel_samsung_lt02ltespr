...
static int get_cable_compensation_voltage(
	struct sec_fuelgauge_info *fuelgauge, int vcell, int cable)
{
	union power_supply_propval value;
	int comp_value;

	/* use TA input current limit for VBUS compensation */
	if (cable < 0)
		value.intval = fuelgauge->pdata->charging_current[
	...
	comp_value = comp_value * value.intval / 1000;

	dev_dbg(&fuelgauge->client->dev,
		"%s: cable comp value (%dmV, current %dmA)\n", __func__,
		comp_value, value.intval);
	...
	struct i2c_client *client, int voltage_now, int percentage)
{
	...
	dev_dbg(&client->dev, "%s: reset average voltage as %dmV\n",
...
static void adc_monitor_work(struct work_struct *work)
{
	...
	if (fuelgauge->pdata->monitor_initial_count)
		adc_reset_voltage_avg(fuelgauge->client,
			fuelgauge->info.voltage_now, 100);
...
bool sec_hal_fg_resume(struct i2c_client *client)
{
	...
	fuelgauge->info.voltage_now = adc_get_vcell(client);
	adc_reset_voltage_avg(fuelgauge->client,
		fuelgauge->info.voltage_now, 67);
	fuelgauge->info.voltage_avg = adc_get_avg_vcell(client);
	adc_reset_voltage_ocv(fuelgauge->client, fuelgauge->info.voltage_avg);
	fuelgauge->info.voltage_ocv = adc_get_ocv(client);
	fuelgauge->info.capacity = adc_get_soc(fuelgauge->client);
