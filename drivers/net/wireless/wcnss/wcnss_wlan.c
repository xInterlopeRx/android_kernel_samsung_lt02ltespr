...
#define NVBIN_FILE_NO_FEM			"wlan/prima/WCNSS_qcom_wlan_nv_noFEM.bin"
#define NVBIN_FILE_NO_FEM_NAME_LEN		(sizeof(NVBIN_FILE_NO_FEM) + 1)
...
#if defined(CONFIG_MACH_LT02_SPR) || defined(CONFIG_MACH_LT02_ATT)
	char nvbin_file_name[NVBIN_FILE_NO_FEM_NAME_LEN] = "";

	pr_err("wcnss: system_rev = %02d\n", system_rev);
	if (system_rev < 9)
		snprintf(nvbin_file_name, sizeof(nvbin_file_name), NVBIN_FILE_NO_FEM);
	else
		snprintf(nvbin_file_name, sizeof(nvbin_file_name), NVBIN_FILE);
 
	pr_err("wcnss: wcnss_nvbin_dnld_req: %s will be loaded\n", nvbin_file_name);
	ret = request_firmware(&nv, nvbin_file_name, dev);
 
 	if (ret || !nv || !nv->data || !nv->size) {
		pr_err("wcnss: wcnss_nvbin_dnld_req: request_firmware failed for %s\n"
			   "wcnss: will try again with default file(%s)\n",
				nvbin_file_name, NVBIN_FILE);
#endif
...
static int __init wcnss_wlan_init(void)
{
    ...
    printk(KERN_ERR "wcnss_wlan_init\n") ;
