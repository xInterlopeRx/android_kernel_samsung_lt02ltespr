...
static int snd_pcm_open(struct file *file, struct snd_pcm *pcm, int stream)
{
	int err;
	wait_queue_t wait;
	pr_info("%s, restart_cnt %d \n", __func__,restart_cnt);
			    ...
			    struct snd_pcm_sync_ptr __user *_sync_ptr)
{
	...
	if (runtime->render_flag & SNDRV_NON_DMA_MODE) {
		hw_avail = snd_pcm_playback_hw_avail(runtime);
		pr_info("%s 2 hw_avail[%lu] thre[%lu] restart[%d]\n", __func__,hw_avail,runtime->start_threshold,restart_cnt);
		if ((hw_avail >= runtime->start_threshold)
			&& (runtime->render_flag &
				SNDRV_RENDER_STOPPED)) {
			if (substream->ops->restart){
				pr_info("%s: substream->ops->restart \n", __func__);
